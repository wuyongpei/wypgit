...
<body>
			...
			<div class="modal-content">
				<form id="dic-form" method="get">
					...
					<div class="modal-body">
						...
						<input type="hidden" name="patientID" value="${dic.PatientID }">
						<div class="form-group">
							<label >字典类别</label>
							<input class="form-control" type="text" placeholder="" readonly="readonly" value="${ DictionaryName}">
						</div>
						<div class="form-group">
							<label >字典编码</label>
							<input class="form-control" type="text" placeholder="输入字典编码" name="dictionaryCode" value="${dic.DictionaryCode }">
						</div>
						<div class="form-group">
							<label >字典名称</label>
							<input class="form-control" type="text" placeholder="输入字典名称" name="dictionaryName" id="dictionaryName-edit" onchange="getPinYin();" value="${dic.DictionaryName }">
						</div>
						<div class="form-group">
							<label >字典简拼</label>
							<input class="form-control" type="text" placeholder="" name="simpleCode" id="simpleCode" readonly="readonly" value="${dic.SimpleCode }">
						</div>
						<div class="form-group">
							<label >字典排序</label>
							<input class="form-control" type="text" placeholder="输入字典排序序号" name="orderIndex" id="orderIndex" value="${dic.OrderIndex }">
						</div>
						<div class="form-group">
							<label >备注</label>
							<input class="form-control" type="text" placeholder="" name="comment" value="${dic.Comment }">
						</div>
					</div>
	...
	//获取简拼
	function getPinYin(){
		var dicname=$("#dictionaryName-edit").val();
	...
	function save(){
		$("#dic-form").data('bootstrapValidator').validate();//手动对表单进行校验
		var temp=$("#dic-form").data('bootstrapValidator').isValid();
		if(temp==true){
			var formData=new FormData($("#dic-form")[0]);
			$.ajax({
				url:'update/dictionary',
				type:'POST',
				data:formData,
				dataType:'json',
				async:false,
